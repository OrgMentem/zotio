...
**What this means for you:** the default MCP surface is the facade. If you want the full one-tool-per-endpoint mirror instead, set `PP_MCP_SURFACE` (see the [MCP server guide](../guide/mcp-server.md) and the [MCP tools reference](../reference/mcp-tools.md)).
