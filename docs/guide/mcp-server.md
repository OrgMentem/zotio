...
- **JSON results** (`search`, `sql`, and the library resources: collections, items, bundles, manifests, reading notes) carry a top-level `_zotio_provenance` object — `source: zotero_library`, `trust: untrusted_data`, and a notice naming the content as data. Existing shapes are preserved: `search` keeps its `items`/`count` envelope, `sql` keeps its `rows` object, and resources keep their object shape and `application/json` MIME type. A field named `_zotio_provenance` in the library itself cannot forge it, because the authoritative value is written after decoding.
- **Non-JSON text results** — including mirrored CLI stdout that is not JSON — are wrapped in a preamble plus a per-call nonce-delimited `<<<ZOTERO-DATA …>>>` block, so content cannot close the block and resume as trusted text. Unsafe C0/DEL bytes are neutralized in the same pass (tab, LF, and CR survive); mirrored stdout that *is* valid JSON keeps its shape and is neither nonce-framed nor marker-injected, and JSON encoding escapes any control byte inside it.
...
Bounds: every result stays under 60 KB, a top-level array payload is capped at 50 items, and an oversized object is truncated recursively — its nested arrays reduced to at most 50 entries — with the truncation recorded in the payload. Small object results, such as a compact `sql` row envelope, are bounded by bytes rather than by a 50-row cap. Library resources newly adopt this bound; the native tool bounds are unchanged.

Framing is mitigation, not a boundary. What actually decides whether a model's request mutates anything is the CLI's own write gate: `command_run` forwards the flags the model supplies, and mutating commands preview unless `yes` is passed. There is no allowlist of mutating commands — every runnable command is reachable — so the gate is the boundary. Two commands still sit outside it: the generic `import <resource>` writes whenever its own `dry_run` is false, and `vault resolve` always publishes and ignores `dry_run` entirely, which means a `workflow_submit` **preview** containing a resolve step performs a real write. Treat both as unsafe to call from a model until they are gated.
