...
Framing is mitigation, not a boundary. What actually decides whether a model's request mutates anything is the CLI's own write gate: `command_run` forwards the flags the model supplies, and every mutating command previews unless `yes` is passed. There is no allowlist of mutating commands — every runnable command is reachable — so the gate is the boundary, and it now has no exceptions: a model that withholds `yes` cannot write. `dry_run` overrides `yes`, which is what makes a `workflow_submit` preview safe, since the runner previews each step by injecting `--dry-run`.
