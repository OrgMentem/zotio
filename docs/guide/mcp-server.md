...
Framing is mitigation, not a boundary. What actually decides whether a model's request mutates anything is the CLI's own write gate: `command_run` forwards the flags the model supplies, and mutating commands preview unless `yes` is passed. Two commands do not yet honor that gate — `import file` and `import url` write whenever `dry_run` is false — so pass `dry_run: true` to preview those.
