...
A read command may also emit sibling top-level keys beside its own payload, for advisory data it did NOT match. Three commands do this, and they share one naming convention so the grammar is learned once: the rows arrive under `near_<selector>_<thing>`, the outcome under `meta.<selector>_lookup`, and the truncation count under `meta.near_<selector>_total`. `items find --title` reports `near_title_matches` (rows `{key, title, score, year, item_type, trashed}`) with `meta.title_lookup`; `items find --citekey` reports `near_citekey_matches` with `meta.citekey_lookup`; `items duplicates --by title|all` reports `near_title_groups` (rows `{group, score, count, keys, titles, item_type, requires_review}`, where `group` is `title_near`) with `meta.title_lookup`. Rows are strongest first, the score is rounded to two decimals, and `1.00` is reserved for a value that is equal after folding — for a title that means the same title, so a near row is capped below it; for a citekey `1.00` is reachable and means the library holds that key differing only in case, which is the most actionable suggestion of all. A `meta.<selector>_lookup` key names the outcome (`exact_hit`, `near_matches`, `no_near_matches`, `near_lookup_failed`) whenever that selector was itself answered — a returned row matched on it, or the whole lookup matched nothing and the near path ran. It is ABSENT when the question stayed unanswered: the selector was not given, or another selector matched while this one did not, because the selectors are OR-ed and one identifier hit suppresses the report. `items duplicates` adds `not_requested`, because `--by doi` selects a detector that never asks the title question at all, and a stored report must distinguish that silence from a clean library. A `meta.near_<selector>_total` key reports how many candidates were scored when more were scored than listed, and is absent when the list is complete, so "not in these rows" is never mistaken for "not in the library". A sibling key is never folded into the payload, so `.results` and `.groups` keep holding only what the command matched and every existing pipeline is unaffected; these rows are suggestions to confirm, never matches, and `items duplicates resolve` will not merge a near group. Machine formats that have no column for a score (`--plain`, `--csv`) print a one-line stderr note naming the count and `--json` rather than dropping the rows silently; `--quiet` is exempt, because there the exit code is the whole answer. Under `--group all` sibling keys are kept per library at `libraries[].extra` and never merged, because a similarity score only ranks candidates inside the library that produced it.
...
| Flag | Type | Default | Description |
| --- | --- | --- | --- |
| `--by` | `string` | `all` | Duplicate detector to run (doi, title, all); title and all also report titles that are close but not equal, scored and separately (near_title_groups in JSON, with meta.title_lookup naming the state), never as duplicate groups and never as something 'duplicates resolve' will merge |
