...
A read command may also emit sibling top-level keys beside `results`, for advisory data it did NOT match. `items find` is the first: when an exact `--title` lookup matches nothing, it reports `near_title_matches` (each row `{key, title, score, year, item_type, trashed}`, strongest first, score rounded to two decimals and capped below `1.00`, which is reserved for a title that is equal after folding). Two meta keys describe that lookup. `meta.title_lookup` names the outcome (`exact_hit`, `near_matches`, `no_near_matches`, `near_lookup_failed`) whenever the title itself was answered — a returned row matched on the title, or the whole lookup matched nothing and the near path ran. It is ABSENT when the title question stayed unanswered: no `--title` was given, or another selector matched while the title did not, because the selectors are OR-ed and one identifier hit suppresses the title report. `meta.near_title_total` reports how many candidates cleared the similarity floor when more cleared it than the rows listed, and is absent when the list is complete, so "not in these rows" is never mistaken for "not in the library". A sibling key is never folded into `results`, so `.results` keeps holding only what the command matched and every `results[]` pipeline is unaffected; these rows are suggestions to confirm, never matches. Under `--group all` sibling keys are kept per library at `libraries[].extra` and never merged, because a similarity score only ranks candidates inside the library that produced it.
