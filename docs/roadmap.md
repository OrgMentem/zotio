...
- **Phase 3 — Safe remediation.** Findings route to the existing fixers (no new write engine);
  promote `mutate.go` into an `internal/mutation` package only once ≥2 commands consume it; add a
  mutation run-journal; add undo only where reversible.
