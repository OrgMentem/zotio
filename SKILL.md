...
- **`--agent` does NOT auto-apply writes.** It expands to `--json --compact --no-input --no-color` only; mutating commands still preview by default — pass `--yes` to apply. Two exceptions today: `import file` and `import url` write unless you pass `--dry-run`, and `vault push` gates on `--dry-run` rather than `--yes`.
