...
This skill drives the `zotero-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer:
   ```bash
   npx -y @mvanhorn/printing-press install zotero --cli-only
   ```
2. Verify: `zotero-pp-cli --version`
...
- **`vault conflicts`** — List unresolved write-back conflict artifacts.
  ...
  ```
- **`vault resolve`** — Resolve a conflict by citekey or item key. `--keep-vault` republishes the vault copy over the remote (using the live version as a precondition); `--recreate` re-creates a child note that was deleted in Zotero.

  ```bash
  zotero-pp-cli vault resolve smith2024 --keep-vault
  ```
...
- `zotero-pp-cli schema drift` — Detect item-type/field/creator-field changes vs a saved baseline (run after a Zotero upgrade)
...
- `zotero-pp-cli vault conflicts` — List unresolved write-back conflicts
- `zotero-pp-cli vault pull` — Pull remote child-note edits into the vault's `## Notes` region (fast-forward only)
- `zotero-pp-cli vault push` — Write the vault's `## Notes` region back to Zotero child notes
- `zotero-pp-cli vault resolve` — Resolve a write-back conflict (`--keep-vault` / `--recreate`)
- `zotero-pp-cli vault sync` — Export Zotero items to Obsidian/Logseq Markdown notes
...
```bash
zotero-pp-cli which "<capability in your own words>"
```
...
```bash
zotero-pp-cli annotations timeline --since 2026-05-04 --format markdown > this-week.md
```
...
```bash
zotero-pp-cli items missing-pdf --type journalArticle --agent --select data.DOI,data.title | jq '.[] | select(.data.DOI != null) | .data.DOI'
```
...
```bash
zotero-pp-cli items venues --top 20 --agent --select venue,count,year_range
```
...
```bash
zotero-pp-cli auth set-token <key>
```
...
Run `zotero-pp-cli doctor` to verify setup — it reports a `writes:` line (e.g. "available (auto-routed to Web API; reads stay local)", or read-only guidance when no key is set).
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  zotero-pp-cli collections list --agent --select id,name,status
  ```
...
```
zotero-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
zotero-pp-cli feedback --stdin < notes.txt
zotero-pp-cli feedback list --json --limit 10
```

Entries are stored locally at `~/.zotero-pp-cli/feedback.jsonl`. They are never POSTed unless `ZOTERO_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `ZOTERO_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
zotero-pp-cli profile save briefing --json
zotero-pp-cli --profile briefing collections list
zotero-pp-cli profile list --json
zotero-pp-cli profile show briefing
zotero-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `zotero-pp-cli --help` output
...
```bash
claude mcp add zotero-pp-mcp -- zotero-pp-mcp
```
...
1. Check if installed: `which zotero-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   zotero-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `zotero-pp-cli <command> --help`.
