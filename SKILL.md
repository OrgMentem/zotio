...
- **`items bibcheck`** — Check a manuscript (`.tex` or pandoc Markdown) against your library: every `\cite`/`@citekey` resolved, unknown/ambiguous keys flagged, and each unknown key carries the closest library keys as an advisory `suggestions` array (`cite_key`, `item_key`, `title`, `score`) — status stays `unknown`, so a suggestion is never a resolution; `--fail-on-unknown` exits 11 for CI.
...
A read may add **sibling top-level keys** beside `results`. `items find` has two, one for each selector a person types by hand. The exact title test folds case, whitespace, quote and dash styling, and a trailing full stop, so a title pasted from a reference list resolves into `.results`; when it still matches nothing, the command returns `results: []` plus `near_title_matches`, an array of at most five advisory rows (`key`, `title`, `score`, `year`, `item_type`, `trashed`) ordered by similarity score, strongest first. `year` is always present and empty when the item has no parseable date. A missed `--citekey` answers the same way with `near_citekey_matches`, at most three rows of (`cite_key`, `item_key`, `title`, `score`), ranked by edit distance over the whole key with a differing digit costing double a differing letter, because a wrong year is a different work. A citekey is matched exactly as stored, so a row scoring `1.00` means the library holds that key differing only in case. They are suggestions to confirm, never matches, so they sit deliberately outside `.results` — a caller that treats a hit as identity keeps reading exact equality. `--select` and `--compact` do **not** reach them: those apply to `.results` only, so every near row arrives with all of its fields whatever you selected. `items duplicates` answers the same way at library scale: `--by title`/`--by all` return `near_title_groups`, at most 25 rank-capped advisory rows (`group: "title_near"`, `score`, `count`, `keys`, `titles`, `item_type`, `requires_review: true`) that never enter `groups`, are never merged by `duplicates resolve --title`, and are deliberately ignored by `library health` and `library prisma` so published screening counts cannot move; `meta.title_lookup` uses the same four words with one addition, `not_requested`, for `--by doi` where the title detector never ran, and `meta.near_title_total` appears only when more groups were scored than the 25 listed.

`meta.title_lookup` names the outcome, so branch on it rather than on a missing key: `exact_hit` (the title itself matched; `.results` holds it), `near_matches` (`near_title_matches` is populated), `no_near_matches` (nothing in the library is close — the paper is genuinely absent, or the mirror is stale), `near_lookup_failed` (the near search itself broke, and stderr names why; do **not** read this as a confident negative). The key is absent when no `--title` was given, and also when another selector such as `--doi` matched while the title did not: the near lookup never runs once anything matched, so the title question stayed unanswered. `meta.near_title_total` appears only when more candidates cleared the similarity floor than the five rows listed, so absence of your paper from those five is not absence from the library. `meta.citekey_lookup` reports the citekey lookup in exactly the same four words, with `meta.near_citekey_total` for the same truncation reason, and is absent under the same rule — the selectors are OR-ed, so `--doi X --citekey Y` can return rows while the citekey missed. `--plain` and `--csv` build no envelope at all, so those two formats cannot see either set of near rows and print a stderr note pointing at `--json`.
