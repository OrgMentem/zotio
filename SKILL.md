...
This CLI connects directly to your running Zotero desktop app — reads need no API key. Writes (`items create/update/delete/enrich`, `import apply`, `vault push`) generally require a Zotero Web API key, configured once via `auth set-token --stdin`; personal-library creates can instead go through the keyless desktop connector. Mutating commands are preview-first: every one plans by default and applies only under `--yes`. **`--yes` is the user's decision, not yours** — an agent previews, reports, and waits. Runs applied through the mutation envelope are recorded to the journal, but `vault` writes and connector creates bypass it, and `journal undo` reverses only losslessly invertible ops (tag and collection membership) and refuses the rest — so a journal entry is an audit record, not a guarantee of reversibility. See [write safety](https://orgmentem.github.io/zotio/concepts/write-safety/). It syncs your library to a local SQLite store for offline search and compound queries, then layers on the capabilities below — health reports, reviewable import, metadata enrichment, reproducible exports, and vault sync.
...
- **Not every write is journaled, and most are not reversible.** `vault sync/push/pull/resolve` and connector creates never enter the mutation envelope, so `journal undo` cannot see them. Even inside it, only tag and collection-membership changes reverse; field overwrites, merges, enrichment, and permanent deletes do not. Before any multi-item write, take an `export snapshot` — it is the only recovery record for the rest.
- **`--agent` output has three shapes, not one.** Store/API reads wrap payloads in the provenance envelope `{"meta": …, "results": […]}` (`.results` is always an array) — `collections list` is one. Report-shaped commands (`items audit`, `doctor`, `reading-list`, `journal show`, `which`, `schema drift`, `workflow status`) return a bare object with purpose-built top-level keys. List-shaped reports (`items missing-pdf`, `items venues`, `annotations timeline`) return a bare array. Inspect the top-level shape; never assume `.results`. When the envelope is present, `.meta.source` is `live`, `local`, or `live+local` (a de-duplicated union, seen on `items trash`).
...
- **`journal undo`** — Writes routed through the mutation envelope are journaled; `journal undo <run-id>` reverses reversible runs (tag renames, collection moves) and loudly refuses the rest. `vault` writes and connector creates are not journaled at all.
...
```bash
zotio items missing-pdf --type journalArticle --agent --select doi,title | jq -r '.[] | select(.doi != null) | .doi'
```
