...
func TestTagsInventoryAggregationAndCollectionScope(t *testing.T) {
	...
	if _, _, err := db.UpsertBatch("items", []json.RawMessage{
		json.RawMessage(`{"key":"I1","version":1,"data":{"key":"I1","itemType":"book","collections":["COL1"],"tags":[{"tag":" Atlas ","type":0},{"tag":"Mixed","type":0},{"tag":"Only"},{"tag":"Alpha"},{"tag":"   "}]}}`),
		...
		json.RawMessage(`{"key":"CHILD","version":1,"data":{"key":"CHILD","itemType":"attachment","parentItem":"I1","collections":["COL1"],"tags":[{"tag":"Ghost"},{"tag":"Atlas"}]}}`),
	}); err != nil {
		_ = db.Close()
		t.Fatal(err)
	}
	...
	for _, tc := range []struct {
		...
		{"all collections", "", []tagInventoryItem{
			{Tag: "Atlas", CollectionCount: 3, LibraryCount: 4},
		...
		{"COL1", "COL1", []tagInventoryItem{
			{Tag: "Alpha", CollectionCount: 2, LibraryCount: 2, CollectionOnly: true},
			{Tag: "Atlas", CollectionCount: 2, LibraryCount: 4},
			{Tag: "Mixed", CollectionCount: 2, LibraryCount: 3},
			{Tag: "Only", CollectionCount: 2, LibraryCount: 2, CollectionOnly: true},
