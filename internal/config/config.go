...
func Load(configPath string) (*Config, error) {
	...
	// Zotero uses API-key auth. Migrate the old generic auth_header slot into
	// api_key when present, then clear unsupported OAuth/plaintext scaffolding so
	// it cannot shadow the CLI's real credential.
	...
	// Base URL override (used by tests and mock servers)
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("ZOTERO_CONFIG"); path != "" {
...
func (c *Config) SaveUserID(id string) error {
	c.UserID = id
	delete(c.envOverrides, "UserID")
	c.updateFileConfigField("UserID")
	return c.save()
}
...
func cloneVaultConfig(v *VaultConfig) *VaultConfig {
	if v == nil {
		return nil
	}
	out := *v
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["UserID"] {
			out.UserID = c.fileConfig.UserID
		}
	}
