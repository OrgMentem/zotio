...
func TestInverseOps(t *testing.T) {
	...
	if len(refused) != 2 {
		t.Fatalf("refused = %+v, want 2 (duplicate_merge, missing_doi)", refused)
	}
	refusedKeys := map[string]bool{refused[0].Key: true, refused[1].Key: true}
	if !refusedKeys["K4"] || !refusedKeys["K5"] {
		t.Errorf("refused keys = %v, want K4 and K5", refusedKeys)
	}
}
