...
### Changed
- MCP server environment variables were renamed: `PP_MCP_SURFACE` → `ZOTIO_MCP_SURFACE` and `PP_MCP_TRANSPORT` → `ZOTIO_MCP_TRANSPORT`; the legacy `PP_*` names are still honored as deprecated fallbacks.
